...
def generate_question() -> tuple:
    ...
    answer = eval(f'{number1} {operator} {number2}')
    return operator, number1, number2, answer
...
def select_moves(piece, index, moves):
    print(f'Selected to move: {index}.')
    # try:
    #     answer = tkinter.simpledialog.askstring('Question', '1 + 1')
    #     print(answer)
...
pygame.display.set_caption("MathsChess - 2 Player Local")
WHITE = (255, 255, 255)
GREY = (128, 128, 128)
YELLOW = (204, 204, 0)
BLUE = (50, 255, 255)
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if not selected:
                    try:
                        ...
                        print(f'Possible moves: {str(possible)}')

                        if possible:
                            question = generate_question()
                            user_answer = tkinter.simpledialog.askstring(
                                'Question',
                                f'{question[1]} {question[0]} {question[2]} = ?'
                            )

                            print(f'Answer {question[3]}, input: {user_answer}.')

                            if question[3] == int(user_answer):
                                tkinter.messagebox.showinfo('Correct', 'Correct, you can now choose a '
                                                                       'square to move your piece to.')
                                ...
                                tkinter.messagebox.showwarning('Incorrect', f'Incorrect, the answer was {question[3]}. '
                                                                            f'Your piece will move to a random square.')
                                ...
                                board[row][col] = '  '
                                deselect()
                                random_move = random.choice(possible)
                                print(f'random_move = {random_move}')
                                print(f'random_move[1]')
                                print(f'Do_Move(({col}, {row}), ({y}, {x}), WIN)')
                                Do_Move((col, row), (random_move[0], random_move[1]), WIN)
                                moves += 1
                            ...
                            tkinter.messagebox.showerror(
                                title='Error',
                                message='It is either not your turn or you cant move this piece.'
                            )
                    except Exception as e:
                        print(e)
                        piece_to_move = []
                        print('Can\'t select')
                        tkinter.messagebox.showerror(title='Error', message='It is either not your turn or you cant move this piece.')
                    #print(piece_to_move)
